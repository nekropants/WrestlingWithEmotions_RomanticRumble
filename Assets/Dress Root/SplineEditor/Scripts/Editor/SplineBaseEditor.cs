...
namespace Battlehub.SplineEditor
{
    public class SplineBaseEditor : Editor
    {
        ...
        {
            Vector3 point = m_handleTransform.TransformPoint(m_splineBase.GetControlPointLocal(index));
            ...
            if (Handles.Button(point, m_handleRotation, size * HandleSize, size * PickSize, dcf))
            {
                ...
                SplineControlPoint controlPoint = m_splineBase.GetComponentsInChildren<SplineControlPoint>().Where(cpt => cpt.Index == index).FirstOrDefault();
