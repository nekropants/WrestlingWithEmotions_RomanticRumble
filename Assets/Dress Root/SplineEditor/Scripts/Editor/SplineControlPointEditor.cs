            ...
            GUILayout.BeginHorizontal();
            {
                if (curveIndex == m_spline.CurveCount - 1)
                {
                    if (GUILayout.Button("Append"))
                    {
                        ...
                        Selection.activeGameObject = m_spline.GetComponentsInChildren<SplineControlPoint>().Last().gameObject;
                    }
                ...
                if (curveIndex == 0)
                {
                    if (GUILayout.Button("Prepend"))
                    {
                        ...
                        EditorUtility.SetDirty(m_spline);
                        Selection.activeGameObject = m_spline.GetComponentsInChildren<SplineControlPoint>().First().gameObject;
                    }
                }
            }
